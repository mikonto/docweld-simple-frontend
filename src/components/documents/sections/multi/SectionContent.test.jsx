...
vi.mock('@/components/documents/cards', () => ({
  Card: ({ id, title, onRename, onDelete }) => (
  ...
  UploadCard: ({ onUpload, maxFilesAllowed }) => (
  ...
  CardGrid: ({ children }) => <div data-testid="card-grid">{children}</div>,
}));
  ...
  describe('Props Validation', () => {
    ...
    it('should handle documents with various processing states', () => {
      const documentsWithStates = [
        { ...mockProps.documents[0], processingState: 'pending' },
        { ...mockProps.documents[1], processingState: 'processing' },
        {
          id: 'doc3',
          title: 'Document 3',
