...
const mockUseWeldEvents = vi.fn();
const mockCreateEvent = vi.fn();
const mockUseWeldEventOperations = vi.fn();
const mockTimestampFromDate = vi.fn();
const mockUseProjectParticipants = vi.fn();
const mockUseUsers = vi.fn();
const mockToastError = vi.fn();

vi.mock('@/hooks/useWeldEvents', () => ({
  useWeldEvents: (...args: unknown[]) => mockUseWeldEvents(...args),
  useWeldEventOperations: (...args: unknown[]) =>
    mockUseWeldEventOperations(...args),
}));
...
const createMockEvent = (overrides: Partial<WeldEvent> = {}): WeldEvent => ({
...
describe('WeldEventsSection', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockUseWeldEvents.mockReturnValue({
      events: [],
      loading: false,
      error: undefined,
    });
    mockUseWeldEventOperations.mockReturnValue({
  ...
  it('shows loading state when fetching events', () => {
    mockUseWeldEvents.mockReturnValueOnce({
      events: [],
      loading: true,
      error: undefined,
    });

    render(<WeldEventsSection {...baseProps} />);

    expect(screen.getByTestId('weld-events-loading')).toBeInTheDocument();
  });
  ...
  it('renders error state when hook returns error', () => {
    ...
    expect(screen.getByText('weldEvents.errorTitle')).toBeInTheDocument();
  ...
  it('renders creation event when no additional events exist', () => {
    ...
    expect(
      screen.getByText('weldEvents.creation.description')
    ).toBeInTheDocument();
  });
  ...
  it('renders timeline with fetched events and creation entry', () => {
    ...
    expect(
      screen.getByText('weldEvents.eventType.visual-inspection')
    ).toBeInTheDocument();
  ...
  it('allows creating a new event through quick actions for basic user', async () => {
    ...
    await user.click(
      screen.getByRole('menuitem', { name: 'weldEvents.quickActions.logWeld' })
    );
    ...
    await user.click(
      screen.getByRole('button', { name: /weldEvents\.dialogTitles\./ })
    );
  ...
  it('allows admin to select performer constrained by event type', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'weldEvents.quickActions.logEvent' })
    );
    await user.click(
      screen.getByRole('menuitem', {
        name: 'weldEvents.quickActions.logHeatTreatment',
      })
    );
