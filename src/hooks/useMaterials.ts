...
interface UseMaterialOperationsReturn {
  createMaterial: (materialType: MaterialType, materialData: MaterialFormData) => Promise<string>;
  updateMaterial: (materialType: MaterialType, materialId: string, updates: Partial<Material>) => Promise<void>;
  deleteMaterial: (materialType: MaterialType, materialId: string) => Promise<void>;
}
 ...
 *   - Error if any
 */
export const useMaterials = (materialType: MaterialType): [Material[], boolean, FirestoreError | undefined] => {
 ...
 *   - Error if any
 */
export const useAlloyMaterials = (): [Material[], boolean, FirestoreError | undefined] => {
  // Use useFirestoreOperations with active status constraint
...
export const useMaterialOperations = (): UseMaterialOperationsReturn => {
  ...
  const opsMap: Record<MaterialType, ReturnType<typeof useFirestoreOperations>> = {
    parent: parentOps,
    filler: fillerOps,
    alloy: alloyOps,
  };
   ...
   */
  const createMaterial = async (materialType: MaterialType, materialData: MaterialFormData): Promise<string> => {
   ...
   */
  const deleteMaterial = async (materialType: MaterialType, materialId: string): Promise<void> => {
    const ops = opsMap[materialType];
    if (!ops) {
      throw new Error(`Invalid material type: ${materialType}`);
    }
