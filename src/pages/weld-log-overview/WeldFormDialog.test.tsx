...
describe('WeldFormDialog', () => {
  ...
  beforeEach(async () => {
    ...
    mockedUseMaterials.mockImplementation((type) => {
      if (type === 'parent') {
        return [mockMaterials, false, undefined];
      }
      if (type === 'filler') {
        return [mockFillerMaterials, false, undefined];
      }
    ...
    vi.mocked(mockMaterialOperations.createMaterial).mockResolvedValue(
      'new-material-id'
    );
  });
  ...
  describe('Loading States', () => {
    it('should render without crashing when materials are loading', async () => {
      ...
      const mockedUseMaterials = vi.mocked(useMaterials);
      mockedUseMaterials.mockImplementation(() => [[], true, undefined]); // Both materials loading
    ...
    it('should render without crashing when no materials found', async () => {
      const { useMaterials } = await import('@/hooks/useMaterials');
      const mockedUseMaterials = vi.mocked(useMaterials);
      mockedUseMaterials.mockImplementation(() => [[], false, undefined]); // No materials, not loading
