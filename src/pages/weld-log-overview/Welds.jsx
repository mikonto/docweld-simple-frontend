...
const formatMaterialsWithTooltip = (
  ...
  const materialInfos = materialIds.map((id) => {
    ...
    if (material.type) {
      ...
      return `${material.type} - ${material.dimensions || 'N/A'} - ${
        material.alloyMaterial || 'N/A'
      }`;
    } else {
...
export function Welds({
  ...
  const weldColumns = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          {row.getValue('heatTreatment') ? t('common.yes') : t('common.no')}
        </div>
      ),
    },
  ];
