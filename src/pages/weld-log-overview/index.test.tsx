...
vi.mock('./Welds', () => ({
  Welds: ({ onEdit, onCreateNew }: { onEdit: (weld: Weld) => void; onCreateNew: () => void }) => (
...
vi.mock('@/components/documents/cards', () => ({
  ...
  CardGrid: ({ children }: { children: React.ReactNode }) => <div data-testid="document-grid">{children}</div>,
  CardDialog: () => null,
}))
...
describe('WeldLogOverview', () => {
  beforeEach(async () => {
    ...
    mockedUseProject.mockReturnValue([mockProject, false, null])

    const mockedUseWeldLog = vi.mocked(useWeldLog)
    mockedUseWeldLog.mockReturnValue([mockWeldLog, false, null])

    const mockedUseWelds = vi.mocked(useWelds)
    mockedUseWelds.mockReturnValue([mockWelds, false, null])

    const mockedUseWeldLogOperations = vi.mocked(useWeldLogOperations)
    mockedUseWeldLogOperations.mockReturnValue({ updateWeldLog: vi.fn() })
    ...
    mockedUseUser.mockReturnValue([mockUser, false, null])
    ...
    const { getConfirmationContent } = await import('@/utils/confirmationContent')
  ...
  it('should show loading state', () => {
    const mockedUseProject = vi.mocked(useProject)
    mockedUseProject.mockReturnValue([null, true, null])
  ...
  it('should handle missing weld log gracefully', () => {
    const mockedUseWeldLog = vi.mocked(useWeldLog)
    mockedUseWeldLog.mockReturnValue([null, false, null])
