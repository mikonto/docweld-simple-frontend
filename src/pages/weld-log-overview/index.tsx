...
interface WeldFormSubmissionData extends WeldFormData {
  startNumber?: string
  endNumber?: string
  position?: string
...
export default function WeldLogOverview(): React.ReactElement {
  ...
  const [weldLogFormDialog, setWeldLogFormDialog] = useState<WeldLogFormDialogState>({
    isOpen: false,
    weldLog: null,
  })
  ...
  const handleImportSubmit = async (items: any[]): Promise<void> => {
    try {
      ...
      if (results.errors?.length > 0) {
        ...
        results.errors.forEach(({ item, error }: { item: any; error: string }) => {
  ...
  const handleWeldSubmit = async (data: WeldFormSubmissionData, mode?: string): Promise<void> => {
    try {
      if (mode === 'multiple') {
