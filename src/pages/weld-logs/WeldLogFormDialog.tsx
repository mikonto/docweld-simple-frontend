...
interface WeldLogFormDialogProps {
  open: boolean;
...
export function WeldLogFormDialog({
  ...
  const form = useForm<WeldLogFormData>({
  ...
  useEffect(() => {
    if (open) {
      if (weldLog) {
        ...
        const formData: WeldLogFormData = {
          name: weldLog.name || '',
  ...
  const handleSubmit = form.handleSubmit(async (data: WeldLogFormData) => {
    try {
      await onSubmit(data);
      form.reset();
      onOpenChange(false);
