...
vi.mock('./WeldLogFormDialog', () => ({
  WeldLogFormDialog: vi.fn(({ open }) =>
    open ? <div data-testid="weld-log-form-dialog" /> : null
  ),
}));

vi.mock('./WeldLogsTable', () => ({
  WeldLogsTable: vi.fn(({ onRowClick }) => (
    <div data-testid="weld-logs-table">
      <button onClick={() => onRowClick({ id: 'log-123' })}>Row Click</button>
    </div>
  )),
}));

vi.mock('@/components/shared/ConfirmationDialog', () => ({
  ConfirmationDialog: vi.fn(({ isOpen }) =>
...
describe('WeldLogs', () => {
  ...
  const mockWeldLogs = [
    { id: '1', name: 'WL-001', description: 'Test weld log 1' },
    { id: '2', name: 'WL-002', description: 'Test weld log 2' },
  ];
  ...
  beforeEach(async () => {
    ...
    useWeldLogs.mockReturnValue([mockWeldLogs, false, null]);
    useWeldLogOperations.mockReturnValue(mockOperations);
    useFormDialog.mockReturnValue(mockFormDialog);
    useConfirmationDialog.mockReturnValue(mockConfirmDialog);
  ...
  it('should show weld logs loading state', async () => {
    ...
    useWeldLogs.mockReturnValue([[], true, null]);
