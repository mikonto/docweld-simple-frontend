...
import type { Document, ImportedDocumentData, ImportedSectionData } from '@/types/database'
...
interface ImportResults {
  ...
  documents?: ImportedDocumentData[]
  errors?: Array<{ item: { title?: string; name?: string }; error: string }>
}
...
export default function WeldOverview(): ReactElement {
  ...
  const { projectId, weldLogId, weldId } = useParams<RouteParams>() as RouteParams
  ...
  const handleImportSubmit = async (items: (ImportedDocumentData | ImportedSectionData)[]): Promise<void> => {
    try {
      const results = await importItems(items, { projectId, weldLogId }) as ImportResults
      ...
      const errorMessage = error instanceof Error ? error.message : t('documents.importError')
      toast.error(errorMessage)
    }
  }

  // Handler for weld form submission
  const handleWeldSubmit = async (data: WeldFormData): Promise<void> => {
    try {
      // Edit existing weld
