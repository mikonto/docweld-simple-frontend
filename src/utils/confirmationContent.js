 ...
 *
 * @param {string} type - Operation type (delete, archive, restore, etc.)
 * @param {boolean} isBulk - Whether it's a bulk operation
 * @param {number} count - Number of items (for bulk operations)
 * @param {Function} t - Translation function from react-i18next
 * @param {string} entityType - Type of entity (projects, users, etc.) for translation keys
 * @returns {Object} Confirmation dialog content with title, description, actionLabel, and actionVariant
 *
 * @example
 ...
 */
export function getConfirmationContent(type, isBulk, count, t, entityType) {
  // Helper function to build translation keys
  const getKey = (suffix) => `${entityType}.${suffix}`;
